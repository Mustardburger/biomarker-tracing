...
### Step 3.2: Calculate cell type specificity based on seismic cell type specificity score
...
# Just to confirm that there are no "unknown" cell types
cell_types = obj[[]]$cell_type_tissue2 %>% unique()
for (c in cell_types) {
  if ("unknown" %in% c) {
    print(c)
  }
...
obj_sscore <- calc_specificity(obj_sce, ct_label_col='cell_type_tissue2', 
...
file_path = glue("{save_path}/tabula_sapiens_seismic.tsv")
